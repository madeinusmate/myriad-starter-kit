 ...
 * Shows market info, outcome, shares, value, and P&L for each position.
 */
...
import { ArrowUpDown, ExternalLink, BarChart3 } from "lucide-react";
...
function SortIcon({ field, currentField, order }: SortIconProps) {
  ...
  return (
    <ArrowUpDown
      className={cn("ml-2 h-3 w-3", order === "asc" ? "rotate-180" : "")}
    />
  );
}
...
export function PositionsTable({ positions, className }: PositionsTableProps) {
  ...
  return (
    <div
      className={cn("rounded-md border border-border/50 overflow-hidden", className)}
    >
      <Table>
              ...
              return (
                <TableRow
                  ...
                  className={cn(
                    ...
                    isProfitable
                      ? "bg-linear-to-r from-emerald-500/5 via-transparent to-transparent"
                      : "bg-linear-to-r from-rose-500/5 via-transparent to-transparent"
                  )}
                >
                  {/* Market Info */}
                  ...
                  <TableCell className="text-right align-top py-4 text-muted-foreground tabular-nums">
                    {formatCurrency(
                      position.invested ?? position.value - position.profit
                    )}
                  </TableCell>
                  ...
                  <TableCell className="text-right align-top py-4">
                    <div className="flex flex-col items-end gap-0.5">
                      ...
                      >
                        {formatPercent(position.roi, { showSign: true })}
                      </span>
                    </div>
                  </TableCell>
